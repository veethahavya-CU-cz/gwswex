"""GWSWEX demo – deep-GWT → ponding → shallow-GWT column experiment (EXPLICIT solver).
...
Solver: explicit (CFL sub-stepping, dt = 1 h).
...
# ── Build model ───────────────────────────────────────────────────────────────
nc_path = OUTDIR / "demo-explicit.nc"
m = GWSWEXmodel(name="demo-explicit", T="h", L="m", output_fpath=nc_path)
...
m.set_solver(solver="explicit", courant_number=0.9, n_trapz=20, beta_hyst=0.85)
...
fig1.suptitle("GWSWEX demo – loam column GWT transition (explicit solver)", fontsize=11)
...
ax = axs[0]
_shade(ax)
ax.plot(t_d, np.minimum(ts_gwh, Z_TOP), color="#117733", lw=1.8, zorder=2, label="GW head")
...
# Panel 3: Cumulative actual ET (AE / AT / AET)
ax = axs[3]
_shade(ax)
...
for ax2, lbl in zip(axs2, SNAP_ORDER):
    ...
    ax2.plot(x_step, y_step, color="#117733", lw=1.8)
